...
main_loop(){
  ...
  send_spade_command "add storage Quickstep"
    send_spade_command "add filter DropAgents position=1"
    for i in {114..119}; do
    echo "${i}.dot"
    send_spade_command "add reporter Graphviz input=/home/vagrant/trace_dots/${i}.dot"
    while : ; do
    sleep 10
    value=$( grep "Finished reading" "/home/vagrant/SPADE/log/current.log" | wc -l )
